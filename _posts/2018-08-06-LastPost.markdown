...
* Currently known performance considerations
	...
	* In `backward_subst!`, handle the case where you want eigenvectors for conjugate eigenvalues in real arithmetic. WHY? OPTIMAL? STABLE? LAPACK DOES THIS

* Stability considerations for `backward_subst!` such as (scale factor (WHAT IS THIS)), smallest value for which it is safe to divide.
