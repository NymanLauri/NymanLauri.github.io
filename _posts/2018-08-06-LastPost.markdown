...
The same method can be applied to solve the eigenfunctions of the Helmholtz equation. After discretization, the problem takes the form `Ax = λBx`, where matrices `A` and `B` are of size 1176×1176. Below, two eigenfunctions are visualized in a mouth shaped domain with and open boundary condition on left and fixed boundary conditions on the rest of the sides. This is an elementary model of how sound waves can form in the mouth.
